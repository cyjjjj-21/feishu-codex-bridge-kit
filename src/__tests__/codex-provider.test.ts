...
describe('CodexProvider', () => {
  ...
  it('maps started command_execution item to a status progress event', async () => {
    const { CodexProvider } = await import('../codex-provider.js');
    const { PendingPermissions } = await import('../permission-gateway.js');
    const provider = new CodexProvider(new PendingPermissions());
    ...
    (provider as any).handleStartedItem(mockController, {
      type: 'command_execution',
      id: 'cmd-start-1',
      command: 'xcodebuild -project Demo.xcodeproj -scheme Demo build',
    });
    ...
    assert.match(statusData.progress, /^Running command:/);
  });
  ...
  it('honors CTI_CODEX_APPROVAL_POLICY override when set', async () => {
    const old = process.env.CTI_CODEX_APPROVAL_POLICY;
    ...
    try {
      ...
      if (old === undefined) {
        ...
        process.env.CTI_CODEX_APPROVAL_POLICY = old;
      }
    }
  });
  ...
  it('retries once when fresh start fails with stdin-read error before any events', async () => {
    const { CodexProvider } = await import('../codex-provider.js');
    const { PendingPermissions } = await import('../permission-gateway.js');
    const provider = new CodexProvider(new PendingPermissions());
    ...
    const stream = provider.streamChat({
      prompt: 'stdin retry test',
      sessionId: 'stdin-retry-session',
    });
    ...
    assert.equal(startCalls, 2, 'Should retry with a fresh start thread once');
    assert.ok(!errorEvent, 'Retry success should not emit error');
    assert.ok(resultEvent, 'Retry success should emit result');
  });
...
describe('CodexProvider image input', () => {
  ...
  it('prefers an existing stable local image path over creating a temp copy', async () => {
    const { CodexProvider } = await import('../codex-provider.js');
    const { PendingPermissions } = await import('../permission-gateway.js');
    const provider = new CodexProvider(new PendingPermissions());
    ...
    const stablePath = path.join(os.tmpdir(), `cti-stable-${Date.now()}.png`);
    fs.writeFileSync(
      stablePath,
    ...
    try {
      const stream = provider.streamChat({
        ...
        sessionId: 'stable-img-session',
        files: [{ ...makeFile('image/png', 'cG5n', 'stable.png'), filePath: stablePath }],
      });
      ...
      const parts = capturedInput as Array<Record<string, string>>;
      assert.equal(parts[1].type, 'local_image');
      assert.equal(parts[1].path, stablePath);
    } finally {
...
describe('CodexProvider error events', () => {
  ...
  it('can reset transient session runtime state after a stuck run', async () => {
    const { CodexProvider } = await import('../codex-provider.js');
    const { PendingPermissions } = await import('../permission-gateway.js');
    const provider = new CodexProvider(new PendingPermissions());

    (provider as any).threadIds.set('stuck-session', 'thread-123');
    (provider as any).codex = { fake: true };

    provider.resetSessionRuntime?.('stuck-session', { recreateClient: true });

    assert.equal((provider as any).threadIds.has('stuck-session'), false);
