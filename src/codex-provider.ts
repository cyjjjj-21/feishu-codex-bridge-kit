...
function getAdditionalDirectories(): string[] | undefined {
  const configured = process.env.CTI_CODEX_ADDITIONAL_DIRECTORIES
    ?.split(',')
    .map((s) => s.trim())
    .filter(Boolean) ?? [];
  ...
  const dirs = [...configured, ...defaults];
  const unique = Array.from(new Set(dirs)).filter((dir) => {
    try {
...
function shouldRetryFreshThread(message: string): boolean {
  ...
  return (
    ...
    lower.includes('session is busy processing another request') ||
    lower.includes('busy processing another request') ||
...
export class CodexProvider implements LLMProvider {
  ...
  resetSessionRuntime(sessionId: string, opts?: { recreateClient?: boolean }): void {
    this.threadIds.delete(sessionId);
    if (opts?.recreateClient) {
      this.codex = null;
    }
  }
        ...
        (async () => {
          ...
          try {
            ...
            const threadOptions: Record<string, unknown> = {
              ...
              // Codex CLI normally requires a trusted Git repo. The bridge may
              // target a general workspace directory, so skip that preflight.
            ...
            if (imageFiles.length > 0) {
              ...
              for (const file of imageFiles) {
                ...
                if (stablePath && fs.existsSync(stablePath)) {
                  parts.push({ type: 'local_image', path: stablePath });
                  continue;
                }
              ...
              try {
                ...
                for await (const event of events) {
                  ...
                  switch (event.type) {
                    ...
                    case 'turn.started': {
                      controller.enqueue(sseEvent('status', {
                        progress: 'Assistant is working on the task',
                      }));
                      break;
                    }
                    ...
                    case 'item.started': {
                      ...
                      self.handleStartedItem(controller, item);
                      break;
                    }
            ...
            if (shouldResetRuntime || params.abortController?.signal.aborted) {
              self.resetSessionRuntime(params.sessionId, { recreateClient: true });
            }
  ...
  private handleStartedItem(
    ...
    switch (itemType) {
      case 'command_execution': {
        const command = item.command as string || '';
        controller.enqueue(sseEvent('status', {
          progress: `Running command: ${summarizeCommand(command)}`,
        }));
        break;
      }
      ...
      case 'file_change': {
        controller.enqueue(sseEvent('status', {
          progress: 'Applying file changes',
        }));
        break;
      }
