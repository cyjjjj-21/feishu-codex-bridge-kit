...
一个面向飞书 / Codex 的提炼桥接仓库，基于 [op7418/Claude-to-IM-skill](https://github.com/op7418/Claude-to-IM-skill) 和上游 [claude-to-im](https://github.com/op7418/claude-to-im) 做了裁剪、补丁和包装增强。

[English](README.md)

> 署名说明：原始 bridge 架构和多 IM 平台基础能力来自 [op7418/Claude-to-IM-skill](https://github.com/op7418/Claude-to-IM-skill) 及其上游 [claude-to-im](https://github.com/op7418/claude-to-im)。这个仓库只保留桥接相关部分，并加入 Feishu/Codex 实战中的增强修复。
...
- 守护进程是本地进程，没有入站网络监听
- 公开仓库已去除本地凭据和个人环境配置；上线前仍建议自行审查 `config.env`

## 开发

```bash
npm install        # 安装依赖
npm run apply-overrides  # 将本仓 patches 同步到 claude-to-im 依赖
npm run dev        # 开发模式运行
...
`npm install`、`npm test`、`npm run build`、`npm run typecheck` 都会先把这些补丁同步到已安装的 `claude-to-im` 依赖里，再执行实际任务。
