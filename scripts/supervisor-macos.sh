...
macos_prevent_sleep_enabled() {
  local raw
  raw=$(grep "^CTI_MACOS_PREVENT_SLEEP=" "$CTI_HOME/config.env" 2>/dev/null | head -1 | cut -d= -f2- | tr -d "'" | tr -d '"' || true)
  raw="${raw:-true}"
...
build_env_dict() {
  ...
  for var in HOME PATH USER SHELL LANG TMPDIR CODEX_HOME; do
    local val="${!var:-}"
    [ -z "$val" ] && continue
    dict+="${indent}<key>${var}</key>\n${indent}<string>${val}</string>\n"
  done

  # Forward TLS / Node trust overrides when present.
  for var in SSL_CERT_FILE SSL_CERT_DIR NODE_OPTIONS NODE_EXTRA_CA_CERTS; do
    local val="${!var:-}"
    [ -z "$val" ] && continue
    dict+="${indent}<key>${var}</key>\n${indent}<string>${val}</string>\n"
  done
...
generate_plist() {
  ...
  local program_args
  if macos_prevent_sleep_enabled && [ -x "/usr/bin/caffeinate" ]; then
    program_args=$(cat <<ARGS
    <array>
...
supervisor_status_extra() {
  ...
  if macos_prevent_sleep_enabled; then
    echo "macOS sleep prevention: enabled (caffeinate)"
  else
    echo "macOS sleep prevention: disabled"
  fi
}
