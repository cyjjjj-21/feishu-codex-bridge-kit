...
show_feishu_health() {
  ...
  if command -v node >/dev/null 2>&1; then
    ...
    json_runner="/opt/homebrew/bin/node"
  elif [ -x /usr/local/bin/node ]; then
    json_runner="/usr/local/bin/node"
  elif command -v python3 >/dev/null 2>&1; then
    json_runner="python3"
  elif [ -x /usr/bin/python3 ]; then
    json_runner="/usr/bin/python3"
  fi

  if [ -z "$json_runner" ]; then
    echo "Feishu WS health: $FEISHU_HEALTH_FILE"
    return 0
  fi

  if [[ "$json_runner" == *python3 ]]; then
    "$json_runner" - <<'PY' "$FEISHU_HEALTH_FILE"
import json, sys
file = sys.argv[1]
try:
    with open(file, 'r', encoding='utf-8') as f:
        health = json.load(f)
    print('Feishu WS health:')
    print(f"  state: {health.get('wsState') or 'unknown'}")
    print(f"  last ready: {health.get('wsReadyAt') or 'n/a'}")
    print(f"  last inbound: {health.get('lastInboundAt') or 'n/a'}")
    print(f"  last reconnect: {health.get('lastReconnectAt') or 'n/a'}")
    print(f"  reconnects(10m): {health.get('reconnectsLast10Min', 'n/a')}")
    print(f"  watchdog restarts: {health.get('watchdogRestarts', 'n/a')}")
    if health.get('lastWatchdogReason'):
        print(f"  last watchdog reason: {health['lastWatchdogReason']}")
except Exception as err:
    print(f"Feishu WS health: unreadable ({err})")
PY
    return 0
  fi

  "$json_runner" - <<'NODE' "$FEISHU_HEALTH_FILE"
const fs = require('fs');
const file = process.argv[2];
try {
  const health = JSON.parse(fs.readFileSync(file, 'utf8'));
  console.log('Feishu WS health:');
  console.log(`  state: ${health.wsState || 'unknown'}`);
  console.log(`  last ready: ${health.wsReadyAt || 'n/a'}`);
  console.log(`  last inbound: ${health.lastInboundAt || 'n/a'}`);
  console.log(`  last reconnect: ${health.lastReconnectAt || 'n/a'}`);
  console.log(`  reconnects(10m): ${health.reconnectsLast10Min ?? 'n/a'}`);
  console.log(`  watchdog restarts: ${health.watchdogRestarts ?? 'n/a'}`);
  if (health.lastWatchdogReason) {
    console.log(`  last watchdog reason: ${health.lastWatchdogReason}`);
  }
} catch (err) {
  console.log(`Feishu WS health: unreadable (${err instanceof Error ? err.message : String(err)})`);
}
NODE
}
