...
FEISHU_HEALTH_FILE="$CTI_HOME/runtime/feishu-health.json"
...
if [ -f "$CONFIG_FILE" ]; then
  ...
  if echo "$CTI_CHANNELS" | grep -q feishu; then
    ...
    if [ -f "$FEISHU_HEALTH_FILE" ]; then
      check "Feishu WS health snapshot exists" 0

      FEISHU_WS_STATE=$(get_json_field "$FEISHU_HEALTH_FILE" wsState)
      if [ "$FEISHU_WS_STATE" = "ready" ]; then
        check "Feishu WS state is ready" 0
      else
        check "Feishu WS state is ready (current: ${FEISHU_WS_STATE:-unknown})" 1
      fi

      FEISHU_RECONNECTS=$(get_json_field "$FEISHU_HEALTH_FILE" reconnectsLast10Min)
      FEISHU_RECONNECTS="${FEISHU_RECONNECTS:-0}"
      if [ "$FEISHU_RECONNECTS" -lt 3 ] 2>/dev/null; then
        check "Feishu reconnect frequency is healthy (last 10m: $FEISHU_RECONNECTS)" 0
      else
        check "Feishu reconnect frequency is healthy (last 10m: $FEISHU_RECONNECTS)" 1
      fi

      FEISHU_HEALTH_UPDATED=$(get_json_field "$FEISHU_HEALTH_FILE" updatedAt)
      if [ -n "$FEISHU_HEALTH_UPDATED" ]; then
        check "Feishu health snapshot has updatedAt" 0
      else
        check "Feishu health snapshot has updatedAt" 1
      fi
    else
      BRIDGE_RUNNING=0
      ...
      if [ "$BRIDGE_RUNNING" -eq 1 ]; then
        check "Feishu WS health snapshot exists" 1
      else
        check "Feishu WS health snapshot exists (bridge not running, skipped)" 0
      fi
    fi
  fi
...
if [ "$(uname -s)" = "Darwin" ]; then
  if get_bool_config_default_true CTI_MACOS_PREVENT_SLEEP; then
    if command -v caffeinate >/dev/null 2>&1; then
      check "macOS sleep prevention binary available (caffeinate)" 0
    else
      check "macOS sleep prevention binary available (caffeinate)" 1
    fi

    PLIST_FILE="$HOME/Library/LaunchAgents/com.claude-to-im.bridge.plist"
    if [ -f "$PLIST_FILE" ] && grep -q "/usr/bin/caffeinate" "$PLIST_FILE" 2>/dev/null; then
      check "launchd plist wraps bridge with caffeinate" 0
    else
      check "launchd plist wraps bridge with caffeinate (restart bridge if recently changed)" 1
    fi
  else
    check "macOS sleep prevention (disabled by config)" 0
  fi
fi
